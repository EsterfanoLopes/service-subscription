...
func (app *Config) SubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.Session.Put(r.Context(), "error", "bad address.")
	...
	go func() {
		...
		err := pdf.OutputFileAndClose(fmt.Sprintf("%s/%d_manual.pdf", pathToTmp, user.ID))
		if err != nil {
			app.ErrorChan <- err
			return
		}
		...
		msg := mailer.Message{
			...
			AttachmentMap: map[string]string{
				"Manueal.pdf": fmt.Sprintf("%s/%d_manual.pdf", pathToTmp, user.ID),
			},
		}
	...
	if err != nil {
		app.Session.Put(r.Context(), "error", "Error subscribing to plan!bad address.")
